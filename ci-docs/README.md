...
- __related root workflow__ - (`.github/workflows/wfr_*.yml`) has prefix `wfr_`. Workflow which is triggering after some 
specific workflow completion or request. More details [here](https://docs.github.com/en/actions/using-workflows/events-that-trigger-workflows#workflow_run).
_**IMPORTANT** - All updates in such workflow will be applied only if they pushed into default repository branch._
