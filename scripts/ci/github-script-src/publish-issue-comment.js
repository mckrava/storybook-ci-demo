...
module.exports = async ({ github, context, core }) => {
  ...
  if (PUBLISH_ARTIFACTS_LIST === 'true') {
    ...
    if (publishArtifactsWf) {
      ...
      const curlContent = `-X POST -H "Accept: application/vnd.github.v3+json" -H "Authorization: token ${GH_TOKEN}" https://api.github.com/repos/${owner}/${repo}/actions/workflows/${publishArtifactsWf.id}/dispatches -d "{"ref":"${context.payload.repository.default_branch}", "inputs": {"issue_comment_data": ${preparedInputs}}"`;
